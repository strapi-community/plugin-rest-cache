...
import type { FlattenedRoute } from './flattenRoutes';
import type { CachePluginStrategy } from '../../types';
...
/** A middleware entry as Strapi accepts it on a route's config. */
type RouteMiddleware = string | { name: string; config?: Record<string, unknown> };

/** A route with the mutable config Strapi lets plugins extend at register time. */
interface InjectableRoute {
  method?: string;
  path?: string;
  globalPath?: string;
  config?: { middlewares?: RouteMiddleware[] };
}
...
function injectMiddleware(
  ...
  if (typeof route.config.middlewares === 'undefined') {
    ...
    const index = route.config.middlewares.findIndex((middleware) => {
      return (
        middleware === pluginUUid ||
        (typeof middleware === 'object' && middleware.name === pluginUUid)
      );
    });
...
export const injectMiddlewares = function (
  strapi: Core.Strapi,
  ...
  for (const cacheConf of strategy.contentTypes) {
    debug('strapi:plugin-rest-cache')(
      `[REGISTER] ${colors.cyan(cacheConf.contentType)} routes middlewares`
    );
      ...
      if (indexID === -1) {
        ...
        switch (strapiRoutes[indexID].method) {
          ...
          case 'POST':
            ...
            debug('strapi:plugin-rest-cache')(
              `[REGISTER] ${cacheRoute.method} ${cacheRoute.path} ${colors.redBright(
                'purge'
              )}`
            );
            injectMiddleware(strapiRoutes[indexID], 'plugin::rest-cache.purge', {
          ...
          case 'GET': {
            ...
            debug('strapi:plugin-rest-cache')(
              `[REGISTER] GET ${cacheRoute.path} ${colors.green('recv')} ${colors.grey(
                `maxAge=${cacheRoute.maxAge}`
  ...
  if (strategy.enableAdminCTBMiddleware && !purgeViaDocumentService) {
    debug('strapi:plugin-rest-cache')(
      `[REGISTER] ${colors.magentaBright('admin')} routes middlewares`
    );
    let contentMangerRoutes: InjectableRoute[] = [];
    for (const routes of Object.values(strapi.plugins['content-manager'].routes)) {
      for (const route of (routes as { routes: InjectableRoute[] }).routes) {
    ...
    for (const route of adminRoutes.post) {
      ...
      if (indexID !== -1) {
        debug('strapi:plugin-rest-cache')(
          `[REGISTER] POST ${route} ${colors.magentaBright('purge-admin')}`
        );
        injectMiddleware(contentMangerRoutes[indexID], 'plugin::rest-cache.purgeAdmin');
      }
    }
    for (const route of adminRoutes.put) {
      ...
      if (indexID !== -1) {
        debug('strapi:plugin-rest-cache')(
          `[REGISTER] PUT ${route} ${colors.magentaBright('purge-admin')}`
        );
        injectMiddleware(contentMangerRoutes[indexID], 'plugin::rest-cache.purgeAdmin');
      }
    }
