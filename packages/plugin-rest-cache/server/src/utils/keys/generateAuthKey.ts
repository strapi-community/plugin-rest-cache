...
/**
 * The subset of `ctx.state.auth` this key depends on.
 *
 * Deliberately narrow: see the note below on why nothing else from
 * `credentials` may be read here.
 */
interface AuthState {
 ...
 */
export const generateAuthKey = function (ctx: Context): string {
  const auth: AuthState | undefined = ctx.state?.auth;
